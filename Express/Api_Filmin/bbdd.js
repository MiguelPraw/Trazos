
const peliculas = [
    {
        url           : "el-gran-lebowski",
        src           : "https://static.filmin.es/images/media/39491/1/still_1_3_1360x765.png",
        portada       : "https://static.filmin.es/images/media/39491/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/39491/1/still_1_3_1360x765.png",
        titulo        : "El gran Lebowski",
        director      : { nombre : "Joel Coen" , url : "joel-coen"},
        genero        : ["Comedia"],
        edad          : "12+",
        hd            : true,
        nota          : "8,7",
        serie         : false,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1998,
        duracion      : "1h 57min",
        descripcion   : "Los hermanos Coen entretejen una oda ambivalente a la ciudad de Los Ángeles protagonizada por el antihéroe de los 90 por antonomasia, 'El Nota' en este mito incontestable de la cultura pop.",
        detalles    : {
            sinopsis    : "Uno de los mitos de la cultura pop más recordados de la era de los 90, culminación total del cine de los hermanos Coen. 'El gran Lebowski' encierra en sí misma toda la evolución de la comedia contemporánea, desde el humor negro hasta el slapstick pasando por una enfermiza galería de personajes que parecen estar arrancados de las páginas de David Foster Wallace.  Los Coen entretejen una oda ambivalente a la ciudad de Los Ángeles protagonizada por el antihéroe por antonomasia, 'El Nota', magistralmente encarnado por Jeff Bridges. \n\n\nEl Nota (Jeff Bridges), un vago que vive en Los Angeles, un día es confundido por un par de matones con el millonario Jeff Lebowski, con quien sólo comparte apellido. Después de que orinen en su alfombra, el Nota inicia la búsqueda de El Gran Lebowski. De su encuentro surgirá un trato: el Nota recibirá una recompensa si consigue encontrar a la mujer del magnate.",
            premios     : [
                { premio : "EFA"    , cantidad : 1  },
                { premio : "Otros"  , cantidad : 10 }
            ],
            temas       : [ "Clásicos Universal" , "Clásico Moderno" , "Neo-noir" , "Humor Negro" , "Top Clásicos Modernos" ],
            directores  : [ "Joel Coen" ],
            reparto     : [ "Jeff Bridges" , "John Goodman", "Julianne Moore", "Steve Buscemi" ],
            audio       : [ "Español" , "Inglés" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "The Big Lebowski",
                pais            : "Estados Unidos",
                estreno         : "14/05/98",
                recaudacion     : "2.541.775,00 €",
                espectadores    : 690031
            }
        }
    },
    {
        url           : "la-milla-verde",
        src           : "https://static.filmin.es/images/media/39680/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/39680/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/39680/2/still_2_3_1360x765.png",
        titulo        : "La milla verde",
        director      : { nombre : "Frank Darabont" , url : "frank-darabont"},
        genero        : ["Drama"],
        edad          : "13+",
        hd            : true,
        nota          : "8,1",
        serie         : true,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1999,
        duracion      : "3h",
        descripcion   : "Esta adaptación de Stephen King robó los corazones del gran público a finales de los 90 gracias a la impagable química entre Tom Hanks y el descubrimiento de ese año, Michael Clarke Duncan.",
        detalles    : {
            sinopsis    : "Adaptación de una novela de Stephen King que robó los corazones del gran público a finales de los 90 gracias a la insuperable química entre Tom Hanks y el actor revelación, Michael Clarke Duncan. Paul Edgecomb es un funcionario de prisiones encargado de vigilar la 'Milla Verde', un pasillo que separa las celdas de los reclusos condenados a la silla eléctrica. John Coffey, un gigantesco hombre negro acusado de asesinar brutalmente a dos hermanas de nueve años, está esperando su inminente ejecución. Tras una personalidad ingenua e infantil, Coffey esconde un prodigioso don sobrenatural.",
            premios     : [
                { premio : "Oscar"    , cantidad : 4  },
                { premio : "Otros"  , cantidad : 21 }
            ],
            temas       : [ "Clásicos Universal" , "Clásico Moderno" , "Stephen King" , "Obras Maestras" , "Prisiones" ],
            directores  : [ "Frank Darabont" ],
            reparto     : [ "Tom Hanks" , "Michael Clarke Duncan" , "David Morse" , "Bonnie Hunt" ],
            audio       : [ "Español" , "Inglés" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "The Green Mile",
                pais            : "Estados Unidos",
                estreno         : "18/02/00",
                recaudacion     : "2.691.772,00 €",
                espectadores    : 694511
            }
        }
    },
    {
        url           : "el-hombre-que-nunca-estuvo-alli",
        src           : "https://static.filmin.es/images/media/39947/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/39947/8/poster_1_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/39947/2/still_2_3_1360x765.png",
        titulo        : "El hombre que nunca estuvo allí",
        director      : { nombre : "Joel Coen" , url : "joel-coen"},
        genero        : ["Drama" , "Thriller"],
        edad          : "13+",
        hd            : true,
        nota          : "8,4",
        serie         : true,
        suscriptores  : true,
        alquiler      : false,
        anio          : 2001,
        duracion      : "1h 56min",
        descripcion   : "Obra 'noir' tan singular en su forma como rica en su significado que ejemplifica por qué los hermanos Coen componen uno de los duetos más interesantes de la historia del cine.",
        detalles    : {
            sinopsis    : "Obra tan singular en su forma como rica en su contenido que ejemplifica por qué los hermanos Coen componen uno de los duetos más interesantes de la historia del cine. Sirviéndose de unas interpretaciones monumentales de Billy Bob Thornton y Frances McDormand, los Coen abrazan el noir más clásico para elevarlo a su humor cáustico y moderno. Verano de 1949. Ed Crane (Billy Bob Thornton), un introvertido barbero de un pueblecito del norte de California, se siente insatisfecho de su rutinaria vida. Las infidelidades de su mujer (Frances McDormand) le brindan la oportunidad de ejercer un chantaje que podría ayudarle a cambiar su apática existencia.",
            premios     : [
                { premio : "Festival de Cannes" , cantidad : 1  },
                { premio : "Otros"  , cantidad : 32 }
            ],
            temas       : [ "Clásicos Universal" , "Clásico Moderno" , "Cine Negro" , "Neo-noir" , "Cine Policiaco" ],
            directores  : [ "Joel Coen" ],
            reparto     : [ "Billy Bob Thornton" , "Frances McDormand" , "Scarlett Johansson" , "Richard Jenkins" ],
            audio       : [ "Inglés" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "The Man Who Wasn't There",
                pais            : "Estados Unidos",
                estreno         : "12/04/02",
                recaudacion     : "1.797.161,00 €",
                espectadores    : 384248
            }
        }
    },
    {
        url           : "sneakers",
        src           : "https://static.filmin.es/images/media/39820/2/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/39820/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/39820/2/still_2_3_1360x765.png",
        titulo        : "Sneakers (Los fisgones)",
        director      : { nombre : "Phil Alden Robinson" , url : "phil-alden-robinson"},
        genero        : ["Thriller"],
        edad          : "13+",
        hd            : true,
        nota          : "7,3",
        serie         : true,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1992,
        duracion      : "2h 10min",
        descripcion   : "Con un reparto de ensueño en el que sobresalen Robert Redford, esta fantasía tecnológica mostró al mundo las flaquezas de los sistemas de defensa en pleno siglo de la información.",
        detalles    : {
            sinopsis    : "Con un reparto de ensueño en el que sobresalen Robert Redford y Sidney Poitier, esta fantasía tecnológica mostró al mundo las flaquezas de los sistemas de defensa en pleno siglo de la información. Martin Bishop, un genio de los ordenadores, es el líder de un grupo especializado en el mantenimiento de los sistemas de seguridad de grandes empresas. Un día se ve obligado a trabajar para una agencia secreta que le encarga el robo de una caja negra. Pronto averigua que esa caja tiene la capacidad de decodificar cualquier sistema de encriptación y que la agencia que lo ha contratado no es estatal.",
            premios     : [
                { premio : "Edgar Allan Poe Awards" , cantidad : 1  },
                { premio : "Otros"  , cantidad : 1 }
            ],
            temas       : [ "Clásicos Universal" , "Clásico Moderno" , "Cine Politico" , "Robos" , "Espionaje" ],
            directores  : [ "Phil Alden Robinson" ],
            reparto     : [ "Robert Redford" , "Mary McDonnell" , "Sidney Poitier" , "Dan Aykroyd" , "River Phoenix" ],
            audio       : [ "Español" , "Inglés" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "Sneakers",
                pais            : "Estados Unidos",
                estreno         : "21/01/93",
                recaudacion     : "1.704.909,00 €",
                espectadores    : 611460
            }
        }
    },
    {
        url           : "pierrot-el-loco",
        src           : "https://static.filmin.es/images/media/36814/5/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/36814/5/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/36814/12/still_0_3_1360x765.png",
        titulo        : "Pierrot el loco",
        director      : { nombre : "Jean-Luc Godard" , url : "jean-luc-godard" },
        genero        : ["Clásicos" , "Romántica" , "Drama"],
        edad          : "12+",
        hd            : true,
        nota          : "7,7",
        serie         : false,
        suscriptores  : false,
        alquiler      : true,
        anio          : 1965,
        duracion      : "1h 50min",
        descripcion   : "Una de las películas imprescindibles de Jean-Luc Godard con continuas referencias al arte pop, que amplía su colaboración con el mítico actor Jean-Paul Belmondo y la actriz Anna Karina.",
        detalles    : {
            sinopsis    : "Una de las películas imprescindibles de Jean-Luc Godard que amplía su colaboración con el mítico actor Jean-Paul Belmondo y la actriz Anna Karina. Se basa en la novela 'Obsession' (1962), de Lionel White y cuenta con innumerables referencias al arte pop. Ferdinand Griffon, alias 'Pierrot', huye de París con Marianne, la niñera que ha contratado su esposa. La pareja se dirige al sur de Francia, pero el viaje se torna muy peligroso cuando una banda de gángsters con los que Marianne está implicada, les va pisando los talones.",
            premios     : [
                { premio : "Festival de Venecia" , cantidad : 1  },
                { premio : "Otros"  , cantidad : 3 }
            ],
            temas       : [ "Clásicos" , "Romance" , "Nouvelle Vague" , "Cine Experimental" , "Obras Maestras" ],
            directores  : [ "Jean-Luc Godard" ],
            reparto     : [ "Jean-Paul Belmondo" , "Anna Karina" , "Dirk Sanders" , "Jean-Pierre Léaud" ],
            audio       : [ "Español" , "Francés" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "Pierrot le fou",
                pais            : "Francia",
                estreno         : "30/06/80",
                recaudacion     : "50.503,00 €",
                espectadores    : 45283
            }
        }
    },
    {
        url           : "el-discreto-encanto-de-la-burguesia",
        src           : "https://static.filmin.es/images/media/26781/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/26781/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/26781/5/still_1_3_1360x765.png",
        titulo        : "El discreto encanto de la burguesía",
        director      : { nombre : "Luis Buñuel" , url : "luis-buñuel" },
        genero        : ["Clásicos" , "Comedia"],
        edad          : "TP",
        hd            : true,
        nota          : "7,6",
        serie         : false,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1972,
        duracion      : "1h 41min",
        descripcion   : "Luis Buñuel se convirtió en el primer español en recibir un Oscar gracias a esta comedia surrealista basada en una anécdota personal del productor Serge Silberman.",
        detalles    : {
            sinopsis    : `Luis Buñuel se convirtió en el primer español en recibir un Oscar gracias a "El discreto encanto de la burguesía", una comedia surrealista basada en una anécdota personal del productor Serge Silberman. 
            Don Rafael Costa, embajador de Miranda, y el matrimonio Thévenot están invitados a cenar en casa del matrimonio Sénechal, pero a causa de un malentendido tienen que ir a un restaurante. Cuando llegan, no pueden cenar porque el dueño del lugar ha muerto. A partir de ese momento, las reuniones de este selecto grupo de burgueses se verán siempre interrumpidas por las circunstancias más extrañas, algunas reales y otras fruto de su imaginación.`,
            premios     : [
                { premio : "Oscar" , cantidad : 2  },
                { premio : "Otros"  , cantidad : 6 }
            ],
            temas       : [ "Clásicos" , "Comedia Clasica" , "Onirismo y Surrealismo" , "Cine Experimental" , "Obras Maestras" ],
            directores  : [ "Luis Buñuel" ],
            reparto     : [ "Fernando Rey" , "Paul Frankeur" , "Delphine Seyrig" , "Bulle Ogier" , "Stephane Audran" ],
            audio       : [ "Español" , "Francés" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "Le charme discret de la bourgeoisie",
                pais            : "Francia",
                estreno         : "28/01/83",
                recaudacion     : "327.509,00 €",
                espectadores    : 536246
            }
        }
    },
    {
        url           : "la-gran-belleza",
        src           : "https://static.filmin.es/images/media/9959/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/9959/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/9959/2/still_1_3_1360x765.png",
        titulo        : "La Gran Belleza",
        director      : { nombre : "Paolo Sorrentino" , url : "paolo-sorrentino" },
        genero        : ["Drama"],
        edad          : "12+",
        hd            : true,
        nota          : "8,7",
        serie         : false,
        suscriptores  : false,
        alquiler      : true,
        anio          : 2013,
        duracion      : "2h 21min",
        descripcion   : "Multitud de figuras tejen una trama de relaciones inconsistentes, todos envueltos en una Babilonia desesperada.",
        detalles    : {
            sinopsis    : `Damas de la aristocracia, arribistas, hombres políticos, criminales de alto vuelo, periodistas, actores, nobles decadentes, prelados, artistas e intelectuales -auténticos o supuestos- tejen la trama de relaciones inconsistentes, todos envueltos en una Babilonia desesperada que se agita en los palacios antiguos, las inmensas villas, las terrazas más hermosas de la ciudad. Todos están allí. Y no tienen el mejor aspecto. Jep Gambardella, 65 años, escritor y periodista, indolente y decepcionado, la mirada perpetuamente empapada de gin tonic, asiste a este desfile de una humanidad hueca y descompuesta, poderosa y deprimente. Una atonía moral que da vértigo. Y en segundo plano, Roma, el verano. Espléndido e indiferente. Como una diva muerta.`,
            premios     : [
                { premio : "Oscar" , cantidad : 1  },
                { premio : "Otros"  , cantidad : 27 }
            ],
            temas       : [ "Existencialismo" , "Alta Sociedad" , "Roma" , "Artes Plásticas" , "Obras Maestras" ],
            directores  : [ "Paolo Sorrentino" ],
            reparto     : [ "Toni Servillo" , "Carlo Verdone" , "Sabrina Ferilli" , "Serena Grandi" , "Galatea Ranzi" ],
            audio       : [ "Español" , "Italiano" , "Catalán" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "La Grande Bellezza",
                pais            : "Italia",
                estreno         : "05/12/13",
                recaudacion     : "781.666,00 €",
                espectadores    : 117442
            }
        }
    },
    {
        url           : "tiburon",
        src           : "https://static.filmin.es/images/media/32763/2/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/32763/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/32763/4/still_1_3_1360x765.png",
        titulo        : "Tiburón",
        director      : { nombre : "Steven Spielberg" , url : "steven-spielberg" },
        genero        : ["Clásicos" , "Thriller" , "Terror"],
        edad          : "13+",
        hd            : true,
        nota          : "8,8",
        serie         : false,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1975,
        duracion      : "2h 4min",
        descripcion   : "Steven Spielberg reinventó la idea de blockbuster que nos obligó a pensárnoslo un par de veces antes de meternos en el agua.",
        detalles    : {
            sinopsis    : `"Vamos a necesitar un barco más grande". Steven Spielberg reinventó la idea de blockbuster y cambió el rumbo del cine comercial con esta obra maestra que nos obligó a pensárnoslo un par de veces antes de meternos en el agua. Caso aparte, la memorable música de John Williams que contribuyó a generar una ambientación tenebrosa y llena de suspense que pasaría a la historia. 
            En la costa de un pequeño pueblo del Este de los Estados Unidos, un enorme tiburón ataca a varias personas. Por temor a los nefastos efectos que este hecho podría tener sobre el negocio turístico, el alcalde se niega a cerrar las playas y a difundir la noticia. Pero un nuevo ataque del tiburón termina con la vida de un bañista. Cuando el terror se apodera de todos, un veterano cazador de tiburones, un oceanógrafo y el jefe de la policía local se unen para capturar al escualo.`,
            premios     : [
                { premio : "Oscar" , cantidad : 4  },
                { premio : "Otros"  , cantidad : 21 }
            ],
            temas       : [ "Clasicos" , "Clasicos Modernos" , "Blockbuster" , "Top Clasicos Modernos" , "Obras Maestras" ],
            directores  : [ "Steven Spielberg" ],
            reparto     : [ "Roy Scheider" , "Robert Shaw" , "Richard Dreyfuss" , "Lorraine Gary" , "Murray Hamilton" ],
            audio       : [ "Español" , "Inglés" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "Jaws",
                pais            : "Estados Unidos",
                estreno         : "19/12/75",
                recaudacion     : "2.765.261,00 €",
                espectadores    : 5919196
            }
        }
    },
    {
        url           : "la-lista-de-schindler",
        src           : "https://static.filmin.es/images/media/39953/3/card_0_3_550x310.webp",
        portada       : "https://static.filmin.es/images/media/39953/1/poster_0_3_640x0.png",
        fondo         : "https://static.filmin.es/images/media/39953/2/still_2_3_1360x765.png",
        titulo        : "La lista de Schindler",
        director      : { nombre : "Steven Spielberg" , url : "steven-spielberg" },
        genero        : ["Drama"],
        edad          : "13+",
        hd            : true,
        nota          : "9,1",
        serie         : true,
        suscriptores  : false,
        alquiler      : true,
        anio          : 1993,
        duracion      : "3h 16min",
        descripcion   : "Steven Spielberg se consagró en el olimpo del séptimo arte con esta desgarradora historia real del holocausto en la que un empresario alemán salvó miles de vidas judías.",
        detalles    : {
            sinopsis    : `Premiada con 7 Oscars incluyendo el de Mejor Película, Steven Spielberg se consagró en el olimpo del séptimo arte con esta desgarradora historia real del holocausto en la que un empresario alemán sacrificó su negocio y reputación para salvar miles de vidas judías. 
            Oskar Schindler (Liam Neeson), un empresario alemán de gran talento para las relaciones públicas, busca ganarse la simpatía de los nazis de cara a su beneficio personal. Después de la invasión de Polonia por los alemanes en 1939, Schindler consigue, gracias a sus relaciones con los altos jerarcas nazis, la propiedad de una fábrica de Cracovia. Allí emplea a cientos de operarios judíos, cuya explotación le hace prosperar rápidamente, gracias sobre todo a su gerente Itzhak Stern (Ben Kingsley), también judío. Pero conforme la guerra avanza, Schindler y Stern comienzan a ser conscientes de que a los judíos que contratan, los salvan de una muerte casi segura en el temible campo de concentración de Plaszow, que lidera el Comandante nazi Amon Goeth (Ralph Fiennes), un hombre cruel que disfruta ejecutando judíos.`,
            premios     : [
                { premio : "Oscar" , cantidad : 11  },
                { premio : "Otros"  , cantidad : 25 }
            ],
            temas       : [ "Clasicos Universal" , "Clasicos Modernos" , "Nazismo" , "Holocausto" , "II Guerra Mundial" , "Obras Maestras" ],
            directores  : [ "Steven Spielberg" ],
            reparto     : [ "Liam Neeson" , "Ben Kingsley" , "Ralph Fiennes" , "Caroline Goodfall" , "Embeth Davidtz" ],
            audio       : [ "Varios" , "Español" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "Schindler's List",
                pais            : "Estados Unidos",
                estreno         : "04/03/94",
                recaudacion     : "7.025.032,00 €",
                espectadores    : 2332455
            }
        }
    },
    {
        url           : "notting-hill",
        src           : "https://static.filmin.es/images/media/39949/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/39949/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/39949/2/still_2_3_1360x765.png",
        titulo        : "Notting Hill",
        director      : { nombre : "Roger Michell" , url : "roger-michell" },
        genero        : ["Comedia"],
        edad          : "TP",
        hd            : true,
        nota          : "7,8",
        serie         : true,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1999,
        duracion      : "2h 7min",
        descripcion   : "Una de las comedias románticas más conocidas y veneradas de todos los tiempos que inmortalizó a Hugh Grant y a Julia Roberts.",
        detalles    : {
            sinopsis    : `Una de las comedias románticas más conocidas y veneradas de todos los tiempos que inmortalizó a Hugh Grant y a Julia Roberts como los reyes del género. 
            William (Hugh Grant), tranquilo propietario de una pequeña librería de viajes en el popular barrio londinense de Notting Hill, conoce un día por casualidad a la famosa Anna Scott (Julia Roberts), la estrella más rutilante del firmamento cinematográfico actual. Desde el momento en que la actriz entra en su tienda, la vida de William empezará a cambiar.`,
            premios     : [
                { premio : "EFA" , cantidad : 1  },
                { premio : "Otros"  , cantidad : 17 }
            ],
            temas       : [ "Clasicos Universal" , "Clasicos Modernos" , "Comedia Romantica" , "Romance" , "Grandes Romances" ],
            directores  : [ "Roger Michell" ],
            reparto     : [ "Julia Roberts" , "Hugh Grant" , "Richard McCabe" , "Hugh Bonneville" ],
            audio       : [ "Inglés" , "Español" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "Notting Hill",
                pais            : "Reino Unido",
                estreno         : "16/07/99",
                recaudacion     : "12.462.305,00 €",
                espectadores    : 3372989
            }
        }
    },
    {
        url           : "el-presidente-y-miss-wade",
        src           : "https://static.filmin.es/images/media/39377/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/39377/3/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/39377/2/still_2_3_1360x765.png",
        titulo        : "El presidente y Miss Wade",
        director      : { nombre : "Rob Reiner" , url : "rob-reiner" },
        genero        : ["Romántica" , "Comedia"],
        edad          : "TP",
        hd            : true,
        nota          : "7,2",
        serie         : true,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1995,
        duracion      : "1h 54min",
        descripcion   : "Vista como un anticipo de 'El ala oeste de la Casa Blanca' en el que Michael Douglas, Annette Bening, Martin Sheen y Michael J. Fox dan vida a otro guion magistral de Aaron Sorkin.",
        detalles    : {
            sinopsis    : `Vista como un anticipo de "El ala oeste de la Casa Blanca" en el que Michael Douglas, Annette Bening, Martin Sheen y Michael J. Fox dan vida a otro guion magistral de Aaron Sorkin.  
            Andrew Shepherd (Michael Douglas), el Presidente de los Estados Unidos, acaba de enviudar. Sin embargo, inicia un inesperado romance con Sydney Wade (Annette Benning), miembro de una importante organización ecologista. En plena campaña electoral, su rival, el republicano Bob Rumson (Richard Dreyfuss), aprovechará esa relación para desacreditarlo.`,
            premios     : [
                { premio : "Oscar" , cantidad : 1  },
                { premio : "Otros"  , cantidad : 7 }
            ],
            temas       : [ "Clasicos Universal" , "Clasicos Modernos" , "Cine Politico" , "Elecciones" , "Grandes Romances" ],
            directores  : [ "Rob Reiner" ],
            reparto     : [ "Michael Douglas" , "Anette Beining" , "Martin Sheen" , "Michael J. Fox" ],
            audio       : [ "Inglés" , "Español" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "The American President",
                pais            : "Estados Unidos",
                estreno         : "27/11/95",
                recaudacion     : "2.229.241,00 €",
                espectadores    : 693585
            }
        }
    },
    {
        url           : "regreso-al-futuro",
        src           : "https://static.filmin.es/images/media/39404/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/39404/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/39404/2/still_2_3_1360x765.png",
        titulo        : "Regreso al futuro",
        director      : { nombre : "Robert Zemeckis" , url : "robert-zemeckis" },
        genero        : ["Clásicos" , "Aventuras" , "Ciencia ficción"],
        edad          : "TP",
        hd            : true,
        nota          : "8,8",
        serie         : true,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1985,
        duracion      : "1h 56min",
        descripcion   : "'Si vas a hacer algo, hazlo con estilo'. El Delorean, los años 50 y la inmortal figura de Michael J. Fox, quién puede olvidar la quintaesencia de la era dorada del blockbuster ochentero?",
        detalles    : {
            sinopsis    : `"Si vas a hacer algo, hazlo con estilo". El Delorean, los años 50, profanar a Chuck Berry o la inmortal figura de Michael J. Fox, ¿quién puede olvidar la quintaesencia de la era dorada del blockbuster ochentero? Steven Spielberg a la producción y Robert Zemeckis a los mandos nos hicieron viajar a la América de los años 50 y de paso nos regalaron uno de los duetos más venerados de la cultura pop: Marty Mcfly y Emmett "Doc" Brown. Imprescindible. 
            El adolescente Marty McFly es amigo de Doc, un científico al que todos toman por loco. Cuando Doc crea una máquina para viajar en el tiempo, un error fortuito hace que Marty llegue a 1955, año en el que sus futuros padres aún no se habían conocido. Después de impedir su primer encuentro, deberá conseguir que se conozcan y se casen; de lo contrario, su existencia no sería posible.`,
            premios     : [
                { premio : "Oscar" , cantidad : 4  },
                { premio : "Otros"  , cantidad : 31 }
            ],
            temas       : [ "Clasicos" , "Clasicos Universal" , "Aventuras" , "Blockbuster" , "Ciencia" ],
            directores  : [ "Rob Reiner" ],
            reparto     : [ "Michael J. Fox" , "Christopher Lloyd" , "Lea Thompson" , "Crispin Glover" ],
            audio       : [ "Inglés" , "Español" ],
            subtitulos  : [ "Español" , "Catalán" ],
            masInfo     : {
                tituloOriginal  : "Back to the Future",
                pais            : "Estados Unidos",
                estreno         : "02/12/85",
                recaudacion     : "5.914.644,00 €",
                espectadores    : 3245885
            }
        }
    },
    {
        url           : "psicosis",
        src           : "https://static.filmin.es/images/media/27465/3/card_0_3_550x310.webp",
        portada       : "https://static.filmin.es/images/media/27465/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/27465/2/still_0_3_1360x765.png",
        titulo        : "Psicosis",
        director      : { nombre : "Alfred Hitchcock" , url : "alfred-hitchcock" },
        genero        : [ "Clasicos" , "Thriller" , "Terror" ],
        edad          : "12+",
        hd            : true,
        nota          : "9,1",
        serie         : false,
        suscriptores  : false,
        alquiler      : true,
        anio          : 1960,
        duracion      : "1h 49min",
        descripcion   : "Probablemente, la película más icónica de la historia del cine, y la más popular del maestro Alfred Hitchcock. Darse una ducha jamás ha vuelto a ser lo mismo.",
        detalles    : {
            sinopsis    : `Probablemente, la película más icónica de la historia del cine, y la más popular del maestro Alfred Hitchcock. Basada en la novela homónima de Robert Bloch, pero muchísimo más influyente que esta, todo el mundo conoce su historia, tiene en mente la célebre escena de la ducha, y ha escuchado la partitura que Bernard Herrmann compuso para el film. Nominada a 4 Oscar, no se llevó ninguno, pero el tiempo la ha colocado en el Olimpo del séptimo arte.
            Marion Crane, una joven secretaria, tras cometer el robo de un dinero en su empresa, huye de la ciudad y, después de conducir durante horas, decide descansar en un pequeño y apartado motel de carretera regentado por un tímido joven llamado Norman Bates, que vive en la casa de al lado con su madre.`,
            premios     : [
                { premio : "Oscar" , cantidad : 4  },
                { premio : "Otros"  , cantidad : 3 }
            ],
            temas       : [ "Clasicos" , "Suspense" , "Terror Psicologico" , "Detectives" , "Obras Maestras" ],
            directores  : [ "Alfred Hitchcock" ],
            reparto     : [ "Anthony Perkins" ],
            audio       : [ "Inglés" , "Español" ],
            subtitulos  : [ "Español" , "Catalán" ],
            masInfo     : {
                tituloOriginal  : "Psycho",
                pais            : "Estados Unidos",
                estreno         : "02/04/61",
                recaudacion     : "No hay datos",
                espectadores    : 0
            }
        }
    },
    {
        url           : "el-pianista",
        src           : "https://static.filmin.es/images/media/17452/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/17452/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/17452/1/still_0_3_1360x765.png",
        titulo        : "El Pianista",
        director      : { nombre : "Roman Polanski" , url : "roman-polanski" },
        genero        : [ "Drama" , "Biopic" ],
        edad          : "13+",
        hd            : true,
        nota          : "9",
        serie         : false,
        suscriptores  : true,
        alquiler      : false,
        anio          : 2002,
        duracion      : "2h 29min",
        descripcion   : "El Holocausto ha sido protagonista de muchas películas. 'El Pianista' es una de las grandes. El enfoque subjetivo de Polanski nos arrastra progresivamente hacia el horror.",
        detalles    : {
            sinopsis    : `El Holocausto ha sido protagonista de muchas películas. "El Pianista" es una de las grandes. El enfoque subjetivo de Polanski nos arrastra progresivamente hacia el horror.Wladyslaw Szpilman, un brillante pianista polaco de origen judío, vive con su familia en el ghetto de Varsovia. Cuando, en 1939, los alemanes invaden Polonia, consigue evitar la deportación gracias a la ayuda de algunos amigos. Pero tendrá que vivir escondido y completamente aislado durante mucho tiempo, y para sobrevivir tendrá que afrontar constantes peligros.`,
            premios     : [
                { premio : "Festival de Cannes" , cantidad : 1 },
                { premio : "Otros"  , cantidad : 19 }
            ],
            temas       : [ "Existencialismo" , "II Guerra Mundial" , "Nazismo" , "Holocausto" , "Obras Maestras" ],
            directores  : [ "Roman Polanski" ],
            reparto     : [ "Adrien Brody" , "Thomas Kretschmann" , "Maureen Lipman" ],
            audio       : [ "Español" , "Catalán" , "Varios" ],
            subtitulos  : [ "Español" , "Catalán" ],
            masInfo     : {
                tituloOriginal  : "The Pianist",
                pais            : "Alemania",
                estreno         : "21/02/17",
                recaudacion     : "7.828.295,00 €",
                espectadores    : 1667019
            }
        }
    },
    {
        url           : "waterworld",
        src           : "https://static.filmin.es/images/media/39946/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/39946/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/39946/1/still_1_3_1360x765.png",
        titulo        : "Waterworld",
        director      : { nombre : "Kevin Reynolds" , url : "kevin-reynolds" },
        genero        : [ "Ciencia ficcion" ],
        edad          : "13+",
        hd            : true,
        nota          : "6,9",
        serie         : false,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1995,
        duracion      : "2h 15min",
        descripcion   : "Imposible olvidar este descomunal blockbuster marítimo denostado en su momento por ser la versión acuática de 'Mad Max' y reivindicada en la actualidad como fenómeno de culto.",
        detalles    : {
            sinopsis    : `Un descomunal blockbuster marítimo cuyo atrevimiento y presupuesto (en su momento fue la película más cara de la historia) casi arruina la carrera de todos los implicados. Criticada por aspirar a ser la versión acuática de "Mad Max" pero reivindicada en la actualidad como fenómeno de culto por su certero retrato del futuro que nos espera, "Waterworld" supuso un cambio de paradigma en la carrera de Kevin Costner, que venía de triunfar con "JFK" o "Bailando con Lobos". 
            Los casquetes polares se han derretido y toda la Tierra está cubierta de agua marina. Los hombres sobreviven en plataformas flotantes y su principal ocupación es la búsqueda de agua dulce, el bien más preciado. Entre ellos circula una leyenda según la cual en algún lugar existe tierra firme. Un viajero errante y solitario que vive del trueque, llega un día a un atolón de chatarra y vende tierra a sus moradores, pero cuando éstos descubren que es un híbrido, mitad pez y mitad humano, lo condenan a muerte.`,
            premios     : [
                { premio : "Oscar" , cantidad : 1 },
                { premio : "Otros"  , cantidad : 5 }
            ],
            temas       : [ "Clasicos Universal" , "Clasico Moderno" , "Aventuras" , "Piratas" , "Distopia" ],
            directores  : [ "Kevin Reynolds" ],
            reparto     : [ "Kevin Costner" , "Dennis Hopper" , "Jeanne Tripplehorn" ],
            audio       : [ "Español" , "Catalán" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "Waterworld",
                pais            : "Estados Unidos",
                estreno         : "08/09/85",
                recaudacion     : "5.562.485,00 €",
                espectadores    : 1756833
            }
        }
    },
    {
        url           : "apocalypse-now",
        src           : "https://static.filmin.es/images/media/33165/1/card_0_3_550x310.png",
        portada       : "https://static.filmin.es/images/media/33165/1/poster_0_3_338x507.png",
        fondo         : "https://static.filmin.es/images/media/33165/5/still_0_3_1360x765.png",
        titulo        : "Apocalypse Now",
        director      : { nombre : "Francis Ford Coppola" , url : "francis-ford-coppola" },
        genero        : [ "Clasicos" , "Aventuras" , "Belico" ],
        edad          : "16+",
        hd            : true,
        nota          : "9,1",
        serie         : false,
        suscriptores  : true,
        alquiler      : false,
        anio          : 1979,
        duracion      : "2h 43min",
        descripcion   : "El montaje de Francis Ford Coppola de su gran película sobre la Guerra de Vietnam. Una obra maestra indiscutible ganadora de dos Oscar y de la Palma de Oro en Cannes.",
        detalles    : {
            sinopsis    : `Último montaje realizado por Francis Ford Coppola de su obra magna, presentado con motivo del 40 aniversario de la película. Su principal diferencia con "Apocalypse Now: Redux" es la duración: en esta ocasión el director ha reducido en 20 minutos el corte final. 
            La película más importante sobre la Guerra de Vietnam. Una obra maestra indiscutible ganadora de dos Oscar y de la Palma de Oro en Cannes que consagró a Francis Ford Coppola como uno de los grandes directores de nuestro tiempo tras también realizar la saga "El Padrino".
            Durante la guerra de Vietnam, al joven Capitán Willard, un oficial de los servicios de inteligencia del ejército estadounidense, se le ha encomendado entrar en Camboya con la peligrosa misión de eliminar a Kurtz, un coronel renegado que se ha vuelto loco. El capitán deberá ir navegar por el río hasta el corazón de la selva, donde parece ser que Kurtz reina como un buda despótico sobre los miembros de la tribu Montagnard, que le adoran como a un dios.`,
            premios     : [
                { premio : "Oscar" , cantidad : 8 },
                { premio : "Otros"  , cantidad : 31 }
            ],
            temas       : [ "Clasicos" , "Guerra de Vietnam" , "Obras Maestras" , "Clasicos Universal" , "Locura" ],
            directores  : [ "Francis Ford Coppola" ],
            reparto     : [ "Martin Sheen" , "Marlon Brando" , "Laurence Fishburne" , "Harrison Ford" ],
            audio       : [ "Inglés" ],
            subtitulos  : [ "Español" ],
            masInfo     : {
                tituloOriginal  : "Apocalypse Now",
                pais            : "Estados Unidos",
                estreno         : "15/04/79",
                recaudacion     : "2.325.641,00 €",
                espectadores    : 2330684
            }
        }
    }
];

const slidersInicio = [
    {
        titulo : "Nuevos Estrenos de Universal",
        subtitulo : "Aquí los clásicos se ven de otra forma",
        items : peliculas,
    },
    {
        titulo : "Las más vistas",
        subtitulo : "De la última semana",
        items : peliculas,
    },
    {
        titulo : "Grandes estrenos exclusivos",
        subtitulo : "Solo los verás en Filmin",
        items : peliculas,
    },
]

const sliderCategorias = [
    {
        src : "https://static.filmin.es/images/wildcard/1358/8/card_0_3_550x311.webp"
    },
    {
        src : "https://static.filmin.es/images/wildcard/1011/1/card_0_3_550x311.webp"
    },
    {
        src : "https://static.filmin.es/images/wildcard/1648/1/card_0_3_550x311.webp"
    },
    {
        src : "https://static.filmin.es/images/wildcard/1507/3/card_0_3_550x311.webp"
    },
    {
        src : "https://static.filmin.es/images/wildcard/1358/8/card_0_3_550x311.webp"
    },
    {
        src : "https://static.filmin.es/images/wildcard/1011/1/card_0_3_550x311.webp"
    },
    {
        src : "https://static.filmin.es/images/wildcard/1648/1/card_0_3_550x311.webp"
    },
    {
        src : "https://static.filmin.es/images/wildcard/1507/3/card_0_3_550x311.webp"
    },
];

const sliderCabecera = [
    {
        inicial   : true,
        video     : false,
        img       : "https://static.filmin.es/images/wildcard/1077/9/highlighted_0_3_1280x605.webp",
        titulo    : "El mayor catálogo de cine y series",
        subtitulo : "Suscríbete por 7,99 €",
        serie     : false,
        boton     : {
            texto : "Más información",
            href : "#"
        }
    },
    {
        video     : true,
        src       : "https://trailer.filmin.com/es/extras/mp4/drive-my-car-trailer-vose-estreno-en-cines.mp4",
        img       : "https://static.filmin.es/images/wildcard/1077/9/highlighted_0_3_1280x605.webp",
        titulo    : "Drive My Car",
        subtitulo : "Ryusuke Hamaguchi",
        serie     : false,
        boton     : {
            texto : "Más información",
            href : "#"
        }
    },
    {
        video     : true,
        src       : "https://trailer.filmin.com/es/extras/mp4/the-beach-bum-tl-fix.mp4",
        img       : "https://static.filmin.es/images/wildcard/1077/9/highlighted_0_3_1280x605.webp",
        titulo    : "The Beach Bum",
        subtitulo : "Harmony Korine",
        serie     : true,
        boton     : {
            texto : "Más información",
            href : "#"
        }
    },
];

module.exports = {
    peliculas,
    slidersInicio,
    sliderCategorias,
    sliderCabecera
}